...
## Usage
Note: in order to run the tool use `java -jar dtmc-gen.jar` or `chmod +x dtmc-gen` and then `./dtmc-gen`
...
The property *statisticalTestConfig* describes the parameters for a statistical test that is used for determining how many samples need to be produced by Perfect Sampling to obtain the step number distribution. 

The choice for the value of *statisticalTestType* is between "**studentTTest**" and "**ZTest**".
...
```
The *distribution* property contains a dictionary where every key/value couple is an entry in the histogram that represents the time step distribution. The keys are the time steps (*x* axis) and the values indicate how many times the algorithm has terminated in that number of steps (*y* axis).
